function [asteroid_names, PermutationMatrix_after, HowMany_after] = sequences_local_pruning3(data_elements,p_number, big_rep_mat)
...
PermutationMatrix_whole = big_rep_mat;
PermutationMatrix_to_cut = PermutationMatrix_whole;

delta_incl_max = 0.5;
% delta_a_max = 0.5;
for i = 1:length(big_rep_mat) % rows
...
for i=1:length(PermutationMatrix_without_e)
    if PermutationMatrix_without_e(i,1) == PermutationMatrix_without_e(i,2)
        PermutationMatrix_without_e(i,:) = "TO BE CUT SAME AST";
    end
end

PermutationMatrix_same_ast = PermutationMatrix_without_e(~all(PermutationMatrix_without_e == "TO BE CUT SAME AST", 2),:); % 2 works by rows, 1 works by columns

PermutationMatrix_after = PermutationMatrix_same_ast(~all(PermutationMatrix_same_ast == "TO BE CUT a", 2),:); % 2 works by rows, 1 works by columns
