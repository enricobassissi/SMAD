...
def refined_selection(dict_risk_list):
    ...
    for key in dict_risk_list.keys():
        if float(dict_risk_list[key]['moid'].scale)<=0.05: #MOID<=0.05 AU
            if (dict_risk_list[key]["magn_radius_flag"]=='H' and float(dict_risk_list[key]["H"])<=28) or (dict_risk_list[key]["magn_radius_flag"]=='D' and float(dict_risk_list[key]["D"])>=20): #low case H is 24.5 meaning 25 m circa
    ...
    for key in dict_risk_list:
            if int(dict_risk_list[key]['N_obs'])>=10: # low case to open -> 5
