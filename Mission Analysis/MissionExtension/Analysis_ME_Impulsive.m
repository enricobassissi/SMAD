...
%% Append multiple 100 days pieces of cloase approach
% --- segment 1
last_ast_name = '2021JE1';
segment_1_date = '2033-01-08';
[date_of_ca_1,cad_objects_2021JE1_1,cad_params_2021JE1_1] = extract_data_ca(module,last_ast_name,...
    segment_1_date);
% --- segment 2
segment_2_date = mjd20002pystr(pystr2mjd2000(segment_1_date)+100);
[date_of_ca_2,cad_objects_2021JE1_2,cad_params_2021JE1_2] = extract_data_ca(module,last_ast_name,...
    segment_2_date);
% --- segment 3 
segment_3_date = mjd20002pystr(pystr2mjd2000(segment_2_date)+100);
[date_of_ca_3,cad_objects_2021JE1_3,cad_params_2021JE1_3] = extract_data_ca(module,last_ast_name,...
    segment_3_date);
% --- segment 4 --- and we got more than 1 year
segment_4_date = mjd20002pystr(pystr2mjd2000(segment_3_date)+100);
[date_of_ca_4,cad_objects_2021JE1_4,cad_params_2021JE1_4] = extract_data_ca(module,last_ast_name,...
    segment_4_date);

% --- append togheter
date_of_ca = [date_of_ca_1;date_of_ca_2;date_of_ca_3;date_of_ca_4];
cad_objects_2021JE1 = [cad_objects_2021JE1_1;cad_objects_2021JE1_2;...
    cad_objects_2021JE1_3;cad_objects_2021JE1_4];
cad_params_2021JE1 = [cad_params_2021JE1_1;cad_params_2021JE1_2;...
    cad_params_2021JE1_3;cad_params_2021JE1_4];

%% call to Analysis of the Mission Extension
end_of_SC2_mission = segment_1_date;
N = 100;
[dvtot_temp,time_vec_contour,time_to_go_vect,base_time_vec,...
    horizons_best_ast_data_temp,TL_2021JE1] = triple_loop_impulsive_ME(sim,N,...
    end_of_SC2_mission,date_of_ca,last_ast_name,cad_objects_2021JE1);
...
for i=1:size(dvtot_temp,3)
    if min(min(dvtot_temp(:,:,i))) < 3
        idx = idx+1;
        dvtot_SC2(:,:,idx) = dvtot_temp(:,:,i);
        cad_objects_SC2(idx,1) = cad_objects_2021JE1(i,1);
        date_of_ca_SC2(idx,1) = date_of_ca(i,1);
        horizons_best_ast_data_SC2{idx} = horizons_best_ast_data_temp{i}; % [x,y,z] in AU; [vx,vy,vz] in AU/day
    end
end
clearvars i idx
        
        
for i = 1:2:size(dvtot_SC2,3)
    figure()
    tiledlayout(2,1)
    ax1 = nexttile;
    contour(time_vec_contour,time_to_go_vect,dvtot_SC2(:,:,i)',[0:0.1:3]);
    cololors2 = colors2(:,:,1);
    colormap(ax1,cololors2)
    hcb = colorbar;
    hcb.Title.String = "\Delta V [km/s]";
    ...
    datetick('x','yyyy mmm dd','keepticks')
    xlim ([time_vec_contour(1) time_vec_contour(end)])
    title(cad_objects_SC2(i))
    
    xline(datenum(datetime(mjd20002date(pystr2mjd2000(date_of_ca_SC2(i))))),'--','LineWidth',2)

    ax2 = nexttile;
    contour(time_vec_contour,time_to_go_vect,dvtot_SC2(:,:,i+1)',[0:0.1:3]);
    cololors2 = colors2(:,:,2);
    colormap(ax2,cololors2)
    hcb = colorbar;
    hcb.Title.String = '\Delta V [km/s]';
    ...
    datetick('x','yyyy mmm dd','keepticks')
    xlim ([time_vec_contour(1) time_vec_contour(end)])
    title(cad_objects_SC2(i+1))
    
    xline(datenum(datetime(mjd20002date(pystr2mjd2000(date_of_ca_SC2(i+1))))),'--','LineWidth',2)
end
clearvars i