function [HS, ANGLES_att_sens, timead, Xad, Xpropad_DT, Xpropad_DT_attitude_sensitivity] = DT_executable_attitude_sensitivity(DT, sim, data)
...
% ---- apply some random noise in the order of 1° around alpha and beta
off_nominal_angle = 1; % deg
alphaHS_attitude_sensitivity = alphaHS + deg2rad(off_nominal_angle*rand(1,length(alphaHS))) - deg2rad(off_nominal_angle/2); % so that it's centered in the middle 
betaHS_attitude_sensitivity = betaHS + deg2rad(off_nominal_angle*rand(1,length(alphaHS))) - deg2rad(off_nominal_angle/2); % so that it's centered in the middle 
ANGLES_att_sens(:,1) = alphaHS_attitude_sensitivity';
ANGLES_att_sens(:,2) = betaHS_attitude_sensitivity';

Xpropad_DT_attitude_sensitivity(1,:) = Xad(1,:);
% RK4 forward integration (in each interval of delta t)
for k = 1:N-1
    xx = Xpropad_DT_attitude_sensitivity(k,:);
%         xx = x(k,:);

    hhh = timead(k+1) - timead(k);
    K1 = EoMpolarAD(timead(k), xx, THS(k), alphaHS_attitude_sensitivity(k), betaHS_attitude_sensitivity(k), data.muS, data);
    K2 = EoMpolarAD(timead(k)+ hhh/2, xx + hhh/2*K1, 0.5*(THS(k+1) + THS(k)), 0.5*(alphaHS_attitude_sensitivity(k+1) + alphaHS_attitude_sensitivity(k)), 0.5*(betaHS_attitude_sensitivity(k)+betaHS_attitude_sensitivity(k+1)), data.muS, data);
    K3 = EoMpolarAD(timead(k)+ hhh/2, xx + hhh/2*K2, 0.5*(THS(k+1) + THS(k)), 0.5*(alphaHS_attitude_sensitivity(k+1) + alphaHS_attitude_sensitivity(k)), 0.5*(betaHS_attitude_sensitivity(k)+betaHS_attitude_sensitivity(k+1)),  data.muS, data);
    K4 = EoMpolarAD(timead(k)+ hhh, xx + hhh*K3, THS(k+1), alphaHS_attitude_sensitivity(k+1), betaHS_attitude_sensitivity(k+1), data.muS, data);    

%         Xprop(k+1,:) = xx + hhh/6*(K1 + 2*K2 + 2*K3 + K4);
    Xpropad_DT_attitude_sensitivity(k+1,:) = Xpropad_DT_attitude_sensitivity(k,:) + hhh/6*(K1 + 2*K2 + 2*K3 + K4);
end
