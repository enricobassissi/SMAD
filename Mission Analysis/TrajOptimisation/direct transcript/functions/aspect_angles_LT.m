function [SAA,EVA,SCA,SolarConjunction,y_sc_EA] = aspect_angles_LT(STR_SA)
...
%}
    ...
    position_wrt_sun = STR_SA.SC_Traj;
    velocity_wrt_sun = STR_SA.SC_Vel;
...
%         rEA_norm = rEA./norm(rEA);
        ...
        SunRadius = astroConstants(3)*1; %km
%         DistSpacecraftEarth = position_wrt_sun(i,:) - rEA';
...
%         && norm(DistSpacecraftEarth) > norm(position_wrt_sun(i,:))
%       we now consider both the superior solar conjunction (sun in
%       between), che the inferior sca (sc earth sun, but rompe il cazzo
%       comunque)
        if abs(SCA(i,1)) < tol 
