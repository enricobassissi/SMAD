...
% TOF1 (2)
bound.TOF1_min = 0.5*365*3600*24/sim.TU; %600
...
bound.lb = [bound.mjd2000_ed, bound.TOF1_min, bound.TOF2_min, ...
    ...
    bound.CT1_min, bound.CT2_min, bound.TOF3_min, bound.N_REV3_min]; % Lower bound
bound.ub = [bound.mjd2000_ld, bound.TOF1_max, bound.TOF2_max, ...
    ...
    bound.CT1_max, bound.CT2_max, bound.TOF3_max, bound.N_REV3_max]; % Upper bound
...
% (4) NREV1, (5) NREV2, (6) IDP, (13) NREV3
constr.IntCon = [4,5,6,13];
...
%% Build the soo
FitnessFunction = @(x) ff_ea_2ast_LT_soo_NLI(x,sim,data); % Function handle to the fitness function
...
sol.mission_duration_d = sol.TOF1+sol.CT1+sol.TOF2+sol.CT2+sol.TOF3;
...
sol.Nrev = [x(4), x(5), x(13)];
sol.v_inf_magn = x(7)*sim.DU/sim.TU;
sol.alpha = x(8);
sol.beta = x(9);
sol.v_launcher = sol.v_inf_magn*[cos(sol.alpha)*cos(sol.beta); sin(sol.alpha)*cos(sol.beta); sin(sol.beta)] ;

%% characteristic quantities plot
[output, r_encounter, v_encounter] = plot_ff_ea_2ast_LT_soo_NLI(x,sim,data);
