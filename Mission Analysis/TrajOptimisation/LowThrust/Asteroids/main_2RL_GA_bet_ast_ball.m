...
%% ------------------------- ARCH 2SC, LT RV --------------------------- %%
%% ------------------------------ SOO ---------------------------------- %%
%% ---- Earth Dep (LT) Ast 1, Coasting, Lambert, GA Earth (LT) Ast 2 --- %%
%% --------------------------------------------------------------------- %%
...
sim.dV_man_max = 0.100/sim.DU*sim.TU; % max dV acceptable at start of Ast1 - GA Leg
...
% azimuth out (POST-GRAVITY ASSIST)(18)
bound.az_out_GA_1_min = -pi;
bound.az_out_GA_1_max =  pi;
% elevation (POST-GRAVITY ASSIST)(19)
...
% azimuth out (POST-GRAVITY ASSIST)(21)
bound.az_out_GA_2_min = -pi;
bound.az_out_GA_2_max =  pi;
% elevation (POST-GRAVITY ASSIST)(22)
...
%      CT1,CTa,(TOFGA,azimuth out,elevation out)x2]
bound.lb = [bound.mjd2000_ed, bound.TOF1_min, bound.TOF2_min, ...
    ...
    bound.TOFGA_1_min, bound.az_out_GA_1_min, bound.el_out_GA_1_min, ...
    bound.TOFGA_2_min, bound.az_out_GA_2_min, bound.el_out_GA_2_min]; % Lower bound
bound.ub = [bound.mjd2000_ld, bound.TOF1_max, bound.TOF2_max, ...
    ...
    bound.TOFGA_1_max, bound.az_out_GA_1_max, bound.el_out_GA_1_max, ...
    bound.TOFGA_2_max, bound.az_out_GA_2_max, bound.el_out_GA_2_max]; % Upper bound
...
%% Build the soo
FitnessFunction = @(x) ff_2RL_GA_bet_ast_ball(x,sim,data,power_propulsion_data); % Function handle to the fitness function
...
sol.TOFGAa = x(20)*sim.TU/86400;
sol.TOFb = x(5)*sim.TU/86400;
sol.TOF1_ADIM = x(2);
sol.TOFGA1_ADIM = x(17);
sol.TOF2_ADIM = x(3);
sol.TOFa_ADIM = x(4);
sol.TOFGAa_ADIM = x(20);
sol.TOFb_ADIM = x(5);
...
[output, r_encounter, v_encounter, sol] = plot_ff_2RL_GA_bet_ast_ball(x,sim,data,sol,power_propulsion_data);
...
legend('show')
xlabel('time from departure [d]'); ylabel('norm(DIST) [AU]'); 
