function [output,r_encounter,v_encounter] = plot_ff_ea_1ast_LT_GA_RV_soo_NLI(x,sim,data)
...
[output.delta_v_p,output.rp]  = flyby(RPlanet_flyby, muPlanet_flyby,R_lim_from_planet, ...
...
output.TOFGA = TOFGA;
output.TOF1 = TOF1;
output.TOF_tot = TOFGA + TOF1;
output.v_inf_magn = x(6);
output.az = x(7);
output.el = x(8);
output.v_launcher = v_launcher;

output.Thrust_magn = sqrt(output.Thrust(:,1).^2 + output.Thrust(:,3).^2);
