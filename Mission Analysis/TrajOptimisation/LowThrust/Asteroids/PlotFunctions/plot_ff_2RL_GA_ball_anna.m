function [output, r_encounter, v_encounter, sol] = plot_ff_2RL_GA_ball_anna(x,sim,data,sol,power_propulsion_data)
...
sol.TOF1 = x(11)*sim.TU/86400;; % tof sc1 GA to Ast1 
sol.CT1 = x(13)*sim.TU/86400;;
sol.TOF2 = x(14)*sim.TU/86400;; % tof sc1 from ast 1 to 2nd asteroid
% 2nd spacecraft characteristic times
% sol.TOF_DSM2 = x(16)*sim.TU/86400;; % It's the time from Earth to DSM for SC2
sol.TOFGA_a = x(17)*sim.TU/86400;; % DSM to GA
sol.TOFa = x(23)*sim.TU/86400;; % tof sc1 GA to Ast1 
sol.CTa = x(25)*sim.TU/86400;;
sol.TOFb = x(26)*sim.TU/86400;; % tof sc1 from ast 1 to 2nd asteroid
% mission duration
...
sol.thetaD1 = x(4); % rad
% -- DSM SC2
sol.thetaD2 = x(16); % rad
% asteorids
sol.asteroid_1 = data.PermutationMatrix(x(28),1);
sol.asteroid_2 = data.PermutationMatrix(x(28),2);
IDP_temp_2 = x(29); % index for 2nd permutation matrix to be built inside depending on the first 2 selected asteroids
...
TOFb = x(27); % tof sc1 from ast 1 to 2nd asteroid
...
IDP_temp_2 = x(30); % index for 2nd permutation matrix to be built inside depending on the first 2 selected asteroids
