...
%% load optimal parameter of previous optim
load('ws_1FL_0.62_pods_42mN.mat')
best.dep_date = sol.departure_date;
best.asteroid_1 = sol.asteroid_1;
best.asteroid_2 = sol.asteroid_2;
best.asteroid_3 = sol.asteroid_3;
best.asteroid_4 = sol.asteroid_4;
...
str_path=split(pwd, 'TrajOptimisation\LowThrust\Asteroids\ZoomIn');
...
% we will use the selected asteroids from the best for now solution and the
% time half year before and half year after

%% Boundaries
best.dep_mjd2000 = date2mjd2000(best.dep_date);
% Departure dates (1)
if (best.dep_mjd2000-180) >= 8765.5 % [2024 1 1 0 0 0]
    bound.mjd2000_ed = (best.dep_mjd2000-180)*3600*24/sim.TU;
else
    bound.mjd2000_ed = 8765.5*3600*24/sim.TU;
end
if (best.dep_mjd2000+180) <= 10226 % [2028 1 1 0 0 0]
    bound.mjd2000_ld = (best.dep_mjd2000+180)*3600*24/sim.TU;
else
    bound.mjd2000_ld = 10226*3600*24/sim.TU;
end
% TOF1 (2)
...
% elevation_ast1_in (24)
bound.el_ast4_min = -pi/2;
bound.el_ast4_max = pi/2;

% x = [MJD0,TOF1,TOF2,TOF3,TOF4,NREV1,NREV2,NREV3,NREV4,v_inf_magn,az,el,
...
bound.lb = [bound.mjd2000_ed, bound.TOF1_min, bound.TOF2_min, ...
    ...
    bound.N_REV3_min, bound.N_REV4_min, ...
...
bound.ub = [bound.mjd2000_ld, bound.TOF1_max, bound.TOF2_max, ...
    ...
    bound.N_REV3_max, bound.N_REV4_max, ...
...
% (6) NREV1, (7) NREV2, (8) NREV3, (9) NREV4
constr.IntCon = [6,7,8,9];
...
%% Build the soo
FitnessFunction = @(x) ff_1FL_ZoomIn(x,sim,data,best); % Function handle to the fitness function
...
sol.v_inf_magn = x(10)*sim.DU/sim.TU;
sol.az = x(11);
sol.el = x(12);


sol.v_inf_ast1_magn = x(13)*sim.DU/sim.TU;
sol.v_inf_ast2_magn = x(16)*sim.DU/sim.TU;
sol.v_inf_ast3_magn = x(19)*sim.DU/sim.TU;
sol.v_inf_ast4_magn = x(22)*sim.DU/sim.TU;


%% characteristic quantities plot and Checks
[output, r_encounter, sol] = plot_ff_1FL_ZoomIn(x,sim,data,sol,best);
...
plot_asteorid_orbit(output.t1(end)*sim.TU/(3600*24),fraction_of_the_orbit,best.asteroid_1,colors,3);
plot_asteorid_orbit(output.t2(end)*sim.TU/(3600*24),fraction_of_the_orbit,best.asteroid_2,colors,4);
plot_asteorid_orbit(output.t3(end)*sim.TU/(3600*24),fraction_of_the_orbit,best.asteroid_3,colors,5);
plot_asteorid_orbit(output.t4(end)*sim.TU/(3600*24),fraction_of_the_orbit,best.asteroid_4,colors,6);
% Sun
...
Traj = [R_transf_orbit_1;R_transf_orbit_2];
max_dist_sun = max(vecnorm(Traj,2,2));
min_dist_sun = min(vecnorm(Traj,2,2));

mjd_earth_plot = [output.t1;output.t2].*sim.TU/86400 + sol.departure_mjd2000;
