...
bound.TOF_DSM1_max = 0.8*365*3600*24/sim.TU; % days
% rD (3)
bound.rD1_min = 0.9; % AU, +/- 20%
bound.rD1_max = 1.1; % AU
% thetaD (4)
bound.thetaD1_min = 0; % rad
bound.thetaD1_max = 1.5*pi; % rad

% GA Stuff SC1, ballistic
% TOFGA (5), from DSM1 to GA 
bound.TOFGA_1_min = 0*365*3600*24/sim.TU; %10*3600
bound.TOFGA_1_max = 0.8*365*3600*24/sim.TU; 
% azimuth out (POST-GRAVITY ASSIST)(6)
bound.az_out_GA_1_min = -pi/2;
bound.az_out_GA_1_max =  pi/2;
% elevation (POST-GRAVITY ASSIST)(7)
bound.el_out_GA_1_min = -pi/6;
bound.el_out_GA_1_max =  pi/6;

% From gravity assist SC1 to ast 1, start of LT powering
% TOF1 (8)
bound.TOF1_min = 1*365*3600*24/sim.TU; %0.2 
...
% TOF2 (11)
bound.TOF2_min = 1*365*3600*24/sim.TU; %0.2
...
bound.TOF_DSM2_max = 0.8*365*3600*24/sim.TU; % days
% rD2 (14)
bound.rD2_min = 0.9; % AU, +/- 20%
bound.rD2_max = 1.1; % AU
% thetaD2 (15)
bound.thetaD2_min = 0; % rad
bound.thetaD2_max = 1.5*pi; % rad

% GA Stuff SC2, ballistic
% TOFGA (16)
bound.TOFGA_2_min = 0*365*3600*24/sim.TU; %10*3600
bound.TOFGA_2_max = 0.8*365*3600*24/sim.TU; 
% azimuth out (POST-GRAVITY ASSIST)(17)
bound.az_out_GA_2_min = -pi/2;
bound.az_out_GA_2_max =  pi/2;
% elevation (POST-GRAVITY ASSIST)(18)
bound.el_out_GA_2_min = -pi/6;
bound.el_out_GA_2_max =  pi/6;
