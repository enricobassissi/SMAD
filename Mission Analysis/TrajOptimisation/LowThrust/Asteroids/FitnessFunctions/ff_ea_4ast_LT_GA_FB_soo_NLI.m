...
%                 disp('1st leg error') % exception of 1st leg wrong
                ...
                if abs(output_1.t(end) - TOF1) > tol_TOF
                    fprintf('help TOF1 %f \n', abs(output_1.t(end) - TOF1))
                end
            end
        else
            obj_fun = obj_fun + 76;
            if (output_GA.t(end) - TOFGA) > tol_TOF
                fprintf('help TOFGA %f \n', abs(output_GA.t(end) - TOFGA))
            end
        end
    else
        obj_fun = obj_fun + 87;
...
%     obj_fun = 1e2*(TOF1+CT1+TOF2+CT2+TOF3 - max_duration)^2;
    fprintf('Long duration %f \n', TOFGA+TOF1+TOF2+TOF3+TOF4)
    obj_fun = obj_fun + 98;
end
