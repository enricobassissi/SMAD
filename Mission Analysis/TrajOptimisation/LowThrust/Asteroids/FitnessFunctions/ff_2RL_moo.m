function obj_fun = ff_2RL_moo(x,sim,data)
%{ 
...
if max(abs(output_a.T_magn)) > sim.max_Available_Thrust
    penalty_T_lega = 10*max(abs(output_a.T_magn));
end
...
if max(abs(output_b.T_magn)) > sim.max_Available_Thrust
    penalty_T_legb = 10*max(abs(output_b.T_magn));
end
...
obj_fun(1) = MF + penalty_MAX_DURATION;
