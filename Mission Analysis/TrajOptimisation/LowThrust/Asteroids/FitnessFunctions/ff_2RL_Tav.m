function obj_fun = ff_2RL_Tav(x,sim,data,power_propulsion_data)
%{ 
...
if ~isnan(output_1.Href)
    % t in years
    t_1 = output_1.t*sim.TU/(3600*24*365);
    % trajectory heliocentric
    r_1  = [output_1.r.*cos(output_1.theta) output_1.r.*sin(output_1.theta) output_1.z];
    R_1 = rotate_local2ecplitic(r_EA,r_1,sim.n_sol,output_1.Href);
    % thrust in heliocentric
    Tlocal_transf_orbit_1  = [-output_1.Thrust(:,1).*sin(output_1.theta), ...
        output_1.Thrust(:,1).*cos(output_1.theta), output_1.Thrust(:,3)];
    Thrust_Helio_1 = rotate_local2ecplitic(r_EA,Tlocal_transf_orbit_1,sim.n_sol,output_1.Href);
    T_magn_Helio_1 = vecnorm(Thrust_Helio_1,2,2).*1e3; % mN
    %thrust available
    T_available = available_thrust(t_1, R_1, Thrust_Helio_1, power_propulsion_data); % mN
    if max(T_magn_Helio_1 - T_available) > 0
        penalty_T_leg1 = max(T_magn_Helio_1 - T_available);
    end
else
    penalty_T_leg1 = max(abs(output_1.T_magn)) - sim.max_Available_Thrust;
end
...
if ~isnan(output_2.Href)
    % t in years
    t_2 = output_2.t*sim.TU/(3600*24*365);
    % trajectory heliocentric
    r_2  = [output_2.r.*cos(output_2.theta) output_2.r.*sin(output_2.theta) output_2.z];
    R_2 = rotate_local2ecplitic(rD1,r_2,sim.n_sol,output_2.Href);
    % thrust in heliocentric
    Tlocal_transf_orbit_2  = [-output_2.Thrust(:,1).*sin(output_2.theta), ...
        output_2.Thrust(:,1).*cos(output_2.theta), output_2.Thrust(:,3)];
    Thrust_Helio_2 = rotate_local2ecplitic(rD1,Tlocal_transf_orbit_2,sim.n_sol,output_2.Href);
    T_magn_Helio_2 = vecnorm(Thrust_Helio_2,2,2).*1e3; % mN
    %thrust available
    T_available = available_thrust(t_2, R_2, Thrust_Helio_2, power_propulsion_data); % mN
    if max(T_magn_Helio_2 - T_available) > 0
        penalty_T_leg2 = max(T_magn_Helio_2 - T_available);
    end
else
    penalty_T_leg2 = max(abs(output_2.T_magn)) - sim.max_Available_Thrust;
end
...
if ~isnan(output_a.Href)
    % t in years
    t_a = output_a.t*sim.TU/(3600*24*365);
    % trajectory heliocentric
    r_a  = [output_a.r.*cos(output_a.theta) output_a.r.*sin(output_a.theta) output_a.z];
    R_a = rotate_local2ecplitic(r_EA,r_a,sim.n_sol,output_a.Href);
    % thrust in heliocentric
    Tlocal_transf_orbit_a  = [-output_a.Thrust(:,1).*sin(output_a.theta), ...
        output_a.Thrust(:,1).*cos(output_a.theta), output_a.Thrust(:,3)];
    Thrust_Helio_a = rotate_local2ecplitic(r_EA,Tlocal_transf_orbit_a,sim.n_sol,output_a.Href);
    T_magn_Helio_a = vecnorm(Thrust_Helio_a,2,2).*1e3; % mN
    %thrust available
    T_available = available_thrust(t_a, R_a, Thrust_Helio_a, power_propulsion_data); % mN
    if max(T_magn_Helio_a - T_available) > 0
        penalty_T_lega = max(T_magn_Helio_a - T_available);
    end
else
...
if ~isnan(output_b.Href)
    % t in years
    t_b = output_b.t*sim.TU/(3600*24*365);
    % trajectory heliocentric
    r_b  = [output_b.r.*cos(output_b.theta) output_b.r.*sin(output_b.theta) output_b.z];
    R_b = rotate_local2ecplitic(rDa,r_b,sim.n_sol,output_b.Href);
    % thrust in heliocentric
    Tlocal_transf_orbit_b  = [-output_b.Thrust(:,1).*sin(output_b.theta), ...
        output_b.Thrust(:,1).*cos(output_b.theta), output_b.Thrust(:,3)];
    Thrust_Helio_b = rotate_local2ecplitic(rDa,Tlocal_transf_orbit_b,sim.n_sol,output_b.Href);
    T_magn_Helio_b = vecnorm(Thrust_Helio_b,2,2).*1e3; % mN
    %thrust available
    T_available = available_thrust(t_b, R_b, Thrust_Helio_b, power_propulsion_data); % mN
    if max(T_magn_Helio_b - T_available) > 0
        penalty_T_legb = max(T_magn_Helio_b - T_available);
    end
else
