function obj_fun = ff_2RL_all_indietro_moo_LW_flex(x,sim,data)
%{ 
...
MJD01 = sim.MJD0_flexibility; % departure time for both the sc

% 1st spacecraft characteristic times
TOF1 = x(1); % tof sc1 to 1st asteroid
MJDA1 = MJD01 + TOF1; % mjd2000 passage of 1st sc on ast 1
CT1 = x(8);
MJDD1 = MJDA1 + CT1;
TOF2 = x(2); % tof sc1 to 2nd asteroid
MJDA2 = MJDD1 + TOF2; % mjd2000 passage of 1st sc on ast 2

% 2nd spacecraft characteristic times
TOFa = x(3); % tof sc2 to 1st asteroid
MJDAa = MJD01 + TOFa; 
CTa = x(9);
MJDDa = MJDAa + CTa;
TOFb =  x(4); % tof sc2 to 2nd asteroid
MJDAb = MJDDa + TOFb; 
...
% IDP1 = x(9); %index of permutation, the column of the Permutation Matrix of the asteroids
% asteroid_1 = data.PermutationMatrix(IDP1,1);
% asteroid_2 = data.PermutationMatrix(IDP1,2);
% 
% % % 2ND SPACECRAFT ASTEROID OBJECTIVES
% IDP_temp_2 = x(10); % index for 2nd permutation matrix to be built inside depending on the first 2 selected asteroids
...
asteroid_1 = data.asteroid_1;
asteroid_2 = data.asteroid_2;
asteroid_a = data.asteroid_a;
asteroid_b = data.asteroid_b;
