...
bound_flex.IDP_min = 1; 
bound_flex.IDP_max = 100; 
...
bound_flex.lb = [bound_flex.mjd2000_ed, bound_flex.TOF1_min, bound_flex.TOF2_min, ...
    ...
    bound_flex.N_REVa_min, bound_flex.N_REVb_min, bound_flex.IDP_min, ...
...
bound_flex.ub = [bound_flex.mjd2000_ld, bound_flex.TOF1_max, bound_flex.TOF2_max, ...
    ...
    bound_flex.N_REVa_max, bound_flex.N_REVb_max, bound_flex.IDP_max, ...
...
options.CreationFcn = @int_pop_2RL_moo_AC_flex;
options.MutationFcn = @int_mutation_2RL_moo_AC_flex;
options.CrossoverFcn = @int_crossoverarithmetic_2RL_moo_AC_flex;

options.PopulationSize = 40; % ideal 1000
...
for i=1:2 % rows, which spacecraft we are considering
    ...
    for j=1:2 % columns, which asteroids that that sc is visiting
        ...
        FitnessFunction = @(x) ff_2RL_all_indietro_moo_AC_flex(x, data, sim); % Function handle to the fitness function
        ...
        FVAL_stored{i,j} = Fval_flex;
        XX_stored{i,j} = xx_flex;
        ...
        if data.IDX_WhichSpacecraftWeAreChanging == 1
            % 1st sc ast change
            IDP_temp = x_flex(10); % index for 2nd permutation matrix to be built inside depending on the first 2 selected asteroids
            HowMany_ThatConfig = length(data.PermutCycle);
            IDP = ceil(IDP_temp*HowMany_ThatConfig/100);
        ...
        elseif data.IDX_WhichSpacecraftWeAreChanging == 2
            ...
            HowMany_ThatConfig = length(data.PermutCycle);
            IDP = ceil(IDP_temp*HowMany_ThatConfig/100);
            asteroid_a = data.PermutCycle(IDP,1);
            asteroid_b = data.PermutCycle(IDP,2);
        end

        flex_order{i,j}.asteroid_sequence_SC1 = [asteroid_1,asteroid_2];
        flex_order{i,j}.asteroid_sequence_SC2 = [asteroid_a,asteroid_b];
        flex_order{i,j}.ast_1 = flex_order{i,j}.asteroid_sequence_SC1(1);
        flex_order{i,j}.ast_2 = flex_order{i,j}.asteroid_sequence_SC1(2);
        flex_order{i,j}.ast_a = flex_order{i,j}.asteroid_sequence_SC2(1);
        flex_order{i,j}.ast_b = flex_order{i,j}.asteroid_sequence_SC2(2);
        flex_order{i,j}.MJD0 = x_flex(1);
        flex_order{i,j}.TOF1 = x_flex(2);
        flex_order{i,j}.TOF2 = x_flex(3);
        flex_order{i,j}.TOFa = x_flex(4);
        flex_order{i,j}.TOFb = x_flex(5);
        flex_order{i,j}.CT1 = x_flex(14);
        flex_order{i,j}.CTa = x_flex(15);
        flex_order{i,j}.dep_date = mjd20002date(x_flex(1))';
        flex_order{i,j}.TOF_tot_D_SC1 = x_flex(2)+x_flex(3)+x_flex(14);
        flex_order{i,j}.TOF_tot_Y_SC1 = flex_order{i,j}.TOF_tot_D_SC1/365;
        flex_order{i,j}.TOF_tot_D_SC2 = x_flex(4)+x_flex(5)+x_flex(15);
        flex_order{i,j}.TOF_tot_Y_SC2 = flex_order{i,j}.TOF_tot_D_SC2/365;
        flex_order{i,j}.end_of_mission_date_SC1 = mjd20002date(x_flex(1)+flex_order{i,j}.TOF_tot_D_SC1)';
        flex_order{i,j}.end_of_mission_date_SC2 = mjd20002date(x_flex(1)+flex_order{i,j}.TOF_tot_D_SC2)';
        
        flex_order{i,j}.Nrev = [x_flex(6),x_flex(7),x_flex(8),x_flex(9)];
        flex_order{i,j}.v_inf_magn = x_flex(11)*sim.DU/sim.TU;
        flex_order{i,j}.az = x_flex(12);
        flex_order{i,j}.el = x_flex(13);
        flex_order{i,j}.az_deg = rad2deg(x_flex(12));
        flex_order{i,j}.el_deg = rad2deg(x_flex(13));
        flex_order{i,j}.obj_fun1 = Fval_flex(idx_knee,1);
        flex_order{i,j}.obj_fun2 = Fval_flex(idx_knee,2);

%         plot_values = plot_ff_2RL_all_indietro_AC_flex(x_flex,sim,data);
%         flex_order{i,j}.max_T = max([max(plot_values.T_1_magn),max(plot_values.T_2_magn),max(plot_values.T_a_magn),max(plot_values.T_b_magn)]);
%         flex_order{i,j}.max_MF = max(plot_values.mass_fract_SC2,plot_values.mass_fract_SC2);

    end
end
el_time=toc;
clearvars i j idx_i idx_j

%% plot the result
for i=1:1 % rows
    for j=1:2 % columns
        MF_flex_order(i,j) = flex_order{i,j}.obj_fun1;
        T_flex_order(i,j) = flex_order{i,j}.obj_fun2;
