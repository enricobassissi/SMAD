...
% Buffer time
sim.moo_lim.buffer_time_min = 30;
sim.moo_lim.buffer_time_max = 180;
% TOF2
sim.moo_lim.TOF2_min = 100; % days
sim.moo_lim.TOF2_max = 3*365; % days

% x = [MJD0, TOF1, v_inf_magn, aplha, beta, buffer_time, TOF2]
sim.moo_bound.lb = [sim.moo_lim.mjd2000_ed, sim.moo_lim.TOF1_min, sim.moo_lim.buffer_time_min,...
      sim.moo_lim.TOF2_min]; % Lower bound
sim.moo_bound.ub = [sim.moo_lim.mjd2000_ld, sim.moo_lim.TOF1_max, sim.moo_lim.buffer_time_max,...
...
options.MaxStallGenerations = 1;

%% Build the moo
FitnessFunction = @ff_neo_without_c3; % Function handle to the fitness function
...
%% Plot trajectories
plot_mission_neo(sol,colors,ast_names)
